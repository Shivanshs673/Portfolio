          ...
          <div className="flex flex-1 flex-col space-y-4 p-4 sm:space-y-5 sm:p-6">
            ...
            <div className="border-t border-white/5 pt-4 mt-3">
              <button
                ...
                className="flex w-full items-center justify-between text-xs font-semibold uppercase tracking-wider text-cyan-400 hover:text-cyan-300 transition-colors cursor-pointer"
              >
              ...
              <AnimatePresence initial={false}>
                {expanded && (
                  ...
                  >
                    <div className="mt-3 space-y-3 pt-1 text-xs leading-relaxed text-slate-300">
                      {project.whyBuilt && (
                        <div>
                          <p className="font-semibold text-slate-200">Why I Built It:</p>
                          <p className="mt-0.5 text-slate-400">{project.whyBuilt}</p>
                        </div>
                      )}
                      {project.keyChallenge && (
                        <div>
                          <p className="font-semibold text-slate-200">Key Technical Challenge:</p>
                          <p className="mt-0.5 text-slate-400">{project.keyChallenge}</p>
                        </div>
                      )}
                      {project.impact && (
                        <div>
                          <p className="font-semibold text-slate-200">Project Impact:</p>
                          <p className="mt-0.5 text-slate-400">{project.impact}</p>
                        </div>
                      )}
                      {project.learnings && (
                        <div>
                          <p className="font-semibold text-slate-200">My Learnings:</p>
                          <p className="mt-0.5 text-slate-400">{project.learnings}</p>
                        </div>
                      )}
                    </div>
                  </motion.div>
